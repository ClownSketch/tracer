...
type mongoSpanDocument struct {
	...
	Status        *mongoStatusDocument        `bson:"status,omitempty"`
	Attributes    map[string]any              `bson:"attributes,omitempty"`
	Events        []bson.M                    `bson:"events,omitempty"`
	Logs          []mongoLogDocument          `bson:"logs,omitempty"`
	Error         *mongoErrorDocument         `bson:"error,omitempty"`
	Resource      *mongoResourceDocument      `bson:"resource,omitempty"`
	ResourceUsage *mongoResourceUsageDocument `bson:"resource_usage,omitempty"`
...
// MongoDBExporterOption MongoDB导出器选项
type MongoDBExporterOption func(*MongoDBExporter)

// WithMongoDBClient 设置MongoDB客户端（如果已存在）
// 注意：如果使用此选项，连接由外部管理，导出器不会关闭连接
func WithMongoDBClient(client *mongo.Client) MongoDBExporterOption {
	return func(e *MongoDBExporter) {
		e.client = client
		e.clientManaged = false // 外部传入的客户端，不由导出器管理
	}
}

// WithMongoDBCollection 设置MongoDB集合（推荐方式，由外部管理连接）
// 这样可以复用已有的连接，支持使用 qmgo 等扩展库
// 注意：如果使用此选项，连接由外部管理，导出器不会关闭连接
func WithMongoDBCollection(collection *mongo.Collection) MongoDBExporterOption {
	return func(e *MongoDBExporter) {
		if collection != nil {
...
// WithMongoDBTimeout 设置操作超时时间
func WithMongoDBTimeout(timeout time.Duration) MongoDBExporterOption {
	return func(e *MongoDBExporter) {
		e.timeout = timeout
	}
}

// WithMongoDBBatchSize 设置批量大小
func WithMongoDBBatchSize(size int) MongoDBExporterOption {
	return func(e *MongoDBExporter) {
		if size > 0 {
			e.batchSize = size
		}
...
// WithMongoDBFlushInterval 设置刷新间隔
func WithMongoDBFlushInterval(interval time.Duration) MongoDBExporterOption {
	return func(e *MongoDBExporter) {
		if interval > 0 {
...
// WithMongoDBQueueSize 保留配置入口。
// 现在异步队列只存在于 BatchProcessor，这里不再维护 exporter 内部队列。
func WithMongoDBQueueSize(size int) MongoDBExporterOption {
	return func(e *MongoDBExporter) {}
}

// WithMongoDBMaxConcurrentWrites 设置最大并发写入数
func WithMongoDBMaxConcurrentWrites(max int) MongoDBExporterOption {
	return func(e *MongoDBExporter) {
		if max > 0 {
...
// WithMongoDBRetries 设置最大重试次数和重试延迟
func WithMongoDBRetries(maxRetries int, retryDelay time.Duration) MongoDBExporterOption {
	return func(e *MongoDBExporter) {
		if maxRetries > 0 {
...
// NewMongoDBExporter 创建MongoDB导出器
// 支持三种方式：
// 1. 使用 WithMongoDBCollection 传入已初始化的 collection（推荐，可复用连接）
//   - 直接使用传入的 collection，导出器不负责关闭连接
//
// 2. 使用 WithMongoDBClient 传入已初始化的 client，配合 database 和 collection 参数
...
func NewMongoDBExporter(uri, database, collection string, opts ...MongoDBExporterOption) (*MongoDBExporter, error) {
	// 初始化导出器默认配置
	e := newMongoDBExporterWithDefaults()
	...
	// 方式1: 如果已经通过 WithMongoDBCollection 设置了 collection，直接复用
	if e.collection != nil {
		return e.initWithCollection()
	}

	// 方式2: 如果已经通过 WithMongoDBClient 设置了 client，复用已有 client
	if e.client != nil {
...
// newMongoDBExporterWithDefaults 创建带有默认配置的 MongoDB 导出器
func newMongoDBExporterWithDefaults() *MongoDBExporter {
	e := &MongoDBExporter{
		batchSize:           50,                     // 默认批量大小
...
// 直接使用传入的 collection，导出器不负责关闭连接
func (e *MongoDBExporter) initWithCollection() (*MongoDBExporter, error) {
	// 连接由外部管理，导出器不负责关闭
...
func (e *MongoDBExporter) initWithClient(database, collection string) (*MongoDBExporter, error) {
	// 验证参数
	if database == "" {
		return nil, fmt.Errorf("使用 WithMongoDBClient 时必须提供 database（数据库名称）")
	}
	if collection == "" {
		return nil, fmt.Errorf("使用 WithMongoDBClient 时必须提供 collection（集合名称）")
	}
...
// 导出器自己连接 mongo，维护连接池，负责关闭连接
// 适用于主项目没有使用 mongo 的场景
func (e *MongoDBExporter) initWithURI(uri, database, collection string) (*MongoDBExporter, error) {
	// 验证参数
...
func (e *MongoDBExporter) validateURIParams(uri, database, collection string) error {
	if uri == "" {
		return fmt.Errorf("MongoDB URI 不能为空（或使用 WithMongoDBCollection/WithMongoDBClient 传入 collection/client）")
	}
	if database == "" {
		return fmt.Errorf("数据库名称不能为空（或使用 WithMongoDBCollection/WithMongoDBClient 传入 collection/client）")
	}
	if collection == "" {
		return fmt.Errorf("集合名称不能为空（或使用 WithMongoDBCollection/WithMongoDBClient 传入 collection）")
	}
	return nil
}

// createMongoClient 创建 MongoDB 客户端连接
func (e *MongoDBExporter) createMongoClient(uri string) (*mongo.Client, error) {
...
func (e *MongoDBExporter) ExportSpan(span trace.SpanSnapshot) error {
	return e.ExportSpans([]trace.SpanSnapshot{span})
}

// ExportSpanSync 同步导出单个 span。
// 该方法会在 MongoDB 真正完成写入（或确认重复键）后返回，适合 WAL 处理器使用。
func (e *MongoDBExporter) ExportSpanSync(ctx context.Context, span trace.SpanSnapshot) error {
	return e.ExportSpansSync(ctx, []trace.SpanSnapshot{span})
}

// ExportSpans 同步批量写入 MongoDB。
// 当前方法只负责把一批 span 写入后端，不负责 fallback，也不释放快照。
func (e *MongoDBExporter) ExportSpans(spans []trace.SpanSnapshot) error {
	if len(spans) == 0 {
		return nil
	}

	items := make([]mongoQueueItem, 0, len(spans))
	validCount := 0
...
// 调用方可以安全推进 WAL ACK。
func (e *MongoDBExporter) ExportSpansSync(ctx context.Context, spans []trace.SpanSnapshot) error {
	if len(spans) == 0 {
		return nil
	}
...
func (e *MongoDBExporter) writeBatchWithRetry(ctx context.Context, items []mongoQueueItem, _ bool) error {
	if len(items) == 0 {
		return nil
	}
...
func (e *MongoDBExporter) insertItemsChunkWithRetry(ctx context.Context, collection *mongo.Collection, timeout time.Duration, items []mongoQueueItem) error {
	...
	for attempt := 0; attempt <= e.maxRetries; attempt++ {
		...
		if err == nil || isDuplicateOnlyInsertError(err) {
			return nil
		}
		...
		if sleepErr := sleepWithContext(ctx, e.retryDelay*time.Duration(attempt+1)); sleepErr != nil {
			return sleepErr
		}
	}
...
func isDuplicateOnlyInsertError(err error) bool {
	...
	if errors.As(err, &bulkErr) {
		...
		for _, writeErr := range bulkErr.WriteErrors {
			if !isDuplicateKeyCode(writeErr.Code) {
				return false
			}
		}
		return true
	...
	if errors.As(err, &writeErr) {
		...
		for _, item := range writeErr.WriteErrors {
			if !isDuplicateKeyCode(item.Code) {
				return false
			}
		}
		return true
...
func (e *MongoDBExporter) buildDocument(span trace.SpanSnapshot, includeRecordID bool) any {
	...
	if includeRecordID {
		doc.RecordID = buildMongoRecordID(span)
	}

	status := span.GetStatus()
	if status.Code != "" || status.Description != "" {
		doc.Status = &mongoStatusDocument{
			Code:        status.Code,
	...
	if logs := span.GetLogs(); len(logs) > 0 {
		logDocs := make([]mongoLogDocument, len(logs))
		for i, log := range logs {
			logDocs[i] = mongoLogDocument{
	...
	if errDetail := span.GetErrorDetail(); errDetail != nil {
		...
		if len(errDetail.StackTrace) > 0 {
			stackTraceDocs := make([]mongoStackFrameDocument, len(errDetail.StackTrace))
			for i, v := range errDetail.StackTrace {
				stackTraceDocs[i] = mongoStackFrameDocument{
					File:         v.File,
					FileName:     v.FileName,
	...
	if resource := span.GetResource(); resource != nil {
		resourceDoc := &mongoResourceDocument{
	...
	if usage := span.GetResourceUsage(); usage != nil {
		usageDoc := &mongoResourceUsageDocument{
	...
	if linkedSpans := span.GetLinkedSpans(); len(linkedSpans) > 0 {
		linkedSpanDocs := make([]mongoLinkedSpanDocument, len(linkedSpans))
		for i, linkedSpan := range linkedSpans {
			linkedSpanDocs[i] = mongoLinkedSpanDocument{
...
//     但为了保持查询灵活性，暂时保留所有索引
func (e *MongoDBExporter) createIndexes() error {
	e.mu.RLock()
	collection := e.collection
	e.mu.RUnlock()
	...
	for _, index := range indexes {
		...
		if err != nil {
			return fmt.Errorf("create mongodb index %q: %w", *indexName, err)
		}
	}
	return nil
}
...
func (e *MongoDBExporter) finishShutdown() {
...
// GetStats 获取统计信息
func (e *MongoDBExporter) GetStats() map[string]int64 {
...
func (e *MongoDBExporter) GetQueueLength() int {
	return 0
}

// GetMaxQueueSize 获取 exporter 队列最大容量。
// exporter 已不再维护内部队列，因此固定返回 0。
func (e *MongoDBExporter) GetMaxQueueSize() int {
	return 0
}
